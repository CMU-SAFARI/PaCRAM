...
namespace Rank {
  ...
  template <class T>
  void RFMab(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk) {
    Rank::debug<T>(node, "------RFMab------", clk);
    auto& cur_power_stats = node->m_spec->m_power_stats[Rank::get_flat_rank_id<T>(node)];
    cur_power_stats.cmd_counters[T::m_cmds_counted("RFMab")]++;
  ...
  template <class T>
  void RFMab_end(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk) {
    Rank::debug<T>(node, "------RFMab_end------", clk);
    auto& cur_power_stats = node->m_spec->m_power_stats[Rank::get_flat_rank_id<T>(node)];
  ...
  template <class T>
  void RRFMab(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk) {
    Rank::debug<T>(node, "------RRFMab------", clk);
    auto& cur_power_stats = node->m_spec->m_power_stats[Rank::get_flat_rank_id<T>(node)];
    cur_power_stats.cmd_counters[T::m_cmds_counted("RRFMab")]++;
  ...
  template <class T>
  void RRFMab_end(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk) {
    Rank::debug<T>(node, "------RRFMab_end------", clk);
    auto& cur_power_stats = node->m_spec->m_power_stats[Rank::get_flat_rank_id<T>(node)];
  ...
  void RFMsb(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk) {
    ...
    cur_power_stats.cmd_counters[T::m_cmds_counted("RFMsb")]++;
    if (is_rank_idle) {
      cur_power_stats.idle_cycles += clk - cur_power_stats.idle_start_cycle;
      cur_power_stats.active_start_cycle = clk;
      std::string msg = "Rank is idle. idle_cycles: " + std::to_string(cur_power_stats.idle_cycles) + "    active_start_cycle: " + std::to_string(cur_power_stats.active_start_cycle);
  ...
  void RRFMsb(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk) {
    ...
    cur_power_stats.cmd_counters[T::m_cmds_counted("RRFMsb")]++;
    if (is_rank_idle) {
      cur_power_stats.idle_cycles += clk - cur_power_stats.idle_start_cycle;
      cur_power_stats.active_start_cycle = clk;
      std::string msg = "Rank is idle. idle_cycles: " + std::to_string(cur_power_stats.idle_cycles) + "    active_start_cycle: " + std::to_string(cur_power_stats.active_start_cycle);
