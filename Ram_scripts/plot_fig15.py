...
for c_id, (config, configdf) in enumerate(sc_df.groupby("config")):
    ...
    for m_id, (mech, mechdf) in enumerate(configdf.groupby("mitigation")):
        ...
        ax.text(0.03, 0.97, mfr, horizontalalignment='left', verticalalignment='top', transform=ax.transAxes, 
...
fig.supylabel('Instruction-per-cycle (IPC)\n(Norm. to Baseline IPC @ $t_{RAS(Nom)}$)', 
                fontsize=7.5, va="center", ha="center", x=0.08, y=0.5)
fig.supxlabel('Charge restoration latency ($t_{RAS(Red)}$), norm. to the nominal charge restoration latency ($t_{RAS(Nom)}$)', 
                fontsize=7.5, x=0.5, y=-0.08)
fig.savefig(PLOT_DIR + "fig15_latency_reduction_sens.png", bbox_inches='tight')
fig.savefig(PLOT_DIR + "fig15_latency_reduction_sens.pdf", bbox_inches='tight')
